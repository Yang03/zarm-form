...
var FORM_ITEM = ['Input', 'Checkbox', 'FilePicker', 'Select', 'CheckboxGroup'];
  ...
  (0, _createClass2.default)(ZarmForm, [{
    ...
    value: function render() {
      ...
      var inputItems = _react.default.Children.map(children, function (element, index) {
        var elemetTypeName = element.type.name;

        if (!FORM_ITEM.includes(elemetTypeName)) {
          console.warn("".concat(elemetTypeName, " is not support, only support ").concat(FORM_ITEM.join(',')));
          return false;
        }
